...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Render recovery from an NPY sample using a combined model."
    )
    add_common_recovery_args(parser)
    parser.set_defaults(
        repo_id="shyamsn97/cube-regen-combined-hdim-20",
        output_dir=Path("examples/recovery/combined"),
        output="combined_recovery.gif",
    )
    parser.add_argument(
        "--config",
        default="configs/train_combined_modal.yaml",
        help="Combined training YAML used for data loading and damage settings.",
    )
    parser.add_argument(
        "--class-label",
        default=None,
...
def main():
    ...
    print(
        f"Selected combined sample: class={label_id} ({class_name}), "
    ...
    run_recovery(
        ...
        args=args,
        description=f"combined {class_name} sample {sample_index}",
    )
